...
export default function ExamCard({ exam }: { exam: Exam }) {
    ...
    return (
        ...
        <p className="text-gray-600">
          Questions per day: <span className="font-medium text-gray-900">12 Q's</span>
        </p>
        <p className="text-gray-600">
          Questions per Month: <span className="font-medium text-gray-900">1 Q's</span>
        </p>
      </div> */}
            ...
            <Link to={`${exam.id}`}>
                <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className={`w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-white font-medium ${colors.button} shadow-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 mb-6`}
                >
