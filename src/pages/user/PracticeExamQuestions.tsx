...
export default function PracticeExamQuestions() {
    ...
    const fetchExamAndExcel = async () => {
      try {
        ...
        if (data.exam_file) {
          ...
          console.log("All Questions:", formattedData);
        }
      } catch (err) {
        console.error(err);
  ...
  // Helper function to infer content type based on the content value
  const inferContentType = (content: string): "audio" | "video" | "text" => {
    if (!content) return "text"; // If content is empty, treat as text

    // Check if content is a URL
    const isUrl = content.startsWith("http://") || content.startsWith("https://");
    if (!isUrl) return "text"; // If not a URL, treat as text
    ...
    // Default to text if the extension is not recognized
    return "text";
  };
        ...
        {questions.length === 0 ? (
          ...
          <div className="space-y-12">
            ...
            {questions.map((question, index) => {
              ...
              const contentType = inferContentType(question.content || "");
              ...
              return (
                ...
                >
                  {/* Render Media Based on inferred content type */}
                  {question.content && (
                    <div className="mb-6">
                      {contentType === "audio" && (
                        <>
