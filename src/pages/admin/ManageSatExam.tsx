...
export default function ManageSatExam() {
    ...
    const handleEdit = (id: number) => {
        navigate(`/admin/import-sat-exam/${id}`)
    }

    const handleDelete = async (id: number) => {
        if(confirm('Are you sure?')){
            try {
                const { error } = await supabase.from('sat_exams').delete().eq('id', id)
    ...
    const openPopUp = () => {
        navigate('/admin/import-sat-exam')
    }
    return (
                            ...
                            <tr key={exam.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <div className="flex gap-2">
                                        ...
                                        <Link to={`/admin/sat-exam-details/${exam.id}`}
