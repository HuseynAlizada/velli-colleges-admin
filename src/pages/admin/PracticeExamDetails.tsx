...
//             const filePath = `practice_exam/${id}.xlsx`;
...
// export default PracticeExamDetails;
...
interface Question {
    Question: string;
    [key: string]: string; // Index signature requiring all properties to be string
    "Correct Option"?: string; // Optional, can be string | undefined
    Section?: string; // Optional, can be string | undefined
}
        ...
        const fetchExamData = async () => {
            try {
                ...
                reader.onload = (e) => {
                    ...
                    const sheetName = workbook.SheetNames[0];
                    const sheet = workbook.Sheets[sheetName];
                    const parsedData = XLSX.utils.sheet_to_json<Question>(sheet);
                    setQuestions(parsedData);

                    const optionKeysSet = new Set<string>();
                    parsedData.forEach((q: Question) => {
                        Object.keys(q).forEach((key) => {
                            if (key.startsWith("Option ")) {
                                optionKeysSet.add(key);
                            }
